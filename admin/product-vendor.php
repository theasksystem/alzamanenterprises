...
$vendor = base64_decode(base64_decode($_GET['vendor']));

if($vendor!=''){

$vendor2 = "and user_id = '$vendor'";
...
if (isset($_GET['visible'])) {
	 ...
	 $result->execute();
	 echo "<script>window.location='product-vendor.php';</script>";
}

if (isset($_GET['featured'])) {
     $result = $conn->prepare("update products set featured = :featured where id = :id");
	 $result->bindParam(':featured', $_GET['featured'], PDO::PARAM_INT);
	 $result->bindParam(':id', $id, PDO::PARAM_INT);
	 $result->execute();
	 echo "<script>window.location='product-vendor.php';</script>";
}
...
$i=0;
$query = $conn->prepare("select * from products where user_id IN(SELECT id FROM `tbl_admin` where type = 'Vendor')  $vendor2 order by id desc");
$query->execute();
$productCount = $query->rowCount();

$productActiveCount2 = $conn->prepare("select * from products where user_id IN(SELECT id FROM `tbl_admin` where type = 'Vendor') and status = 1  $vendor2");
$productActiveCount2->execute();
$productActiveCount = $productActiveCount2->rowCount();

$productInactiveCount2 = $conn->prepare("select * from products where user_id IN(SELECT id FROM `tbl_admin` where type = 'Vendor') and status = 0  $vendor2");
      ...
      <form name="">
                    <select class="form-control" name="vendor" onChange="this.form.submit()">
                      ...
                      <?php 
						...
						$department = $conn->query("SELECT * FROM `tbl_admin` WHERE vis=1 and type = 'Vendor' ORDER BY `username` asc");
						$department->execute();
						while($departmentRow = $department->fetch(PDO::FETCH_ASSOC)){
					  ?>
                      <option <?php if($departmentRow['id']==$vendor){ echo 'selected'; } ?> value="<?=base64_encode(base64_encode($departmentRow['id']));  ?>"><?php echo $departmentRow['username'].'('.$departmentRow['email'].')';  ?></option>
                      <?php } ?>
                    </select>
       </form>
                  </div></li>
          ...
          <table id="example1" width="100%" class="table table-bordered table-striped">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="center"><?php if ($row['featured'] == 1) { ?>
                  <a href="?featured=0&id=<?php echo base64_encode($row['id']); ?>"  title="YES">Yes</a>
                  <?php } else { ?>
                  <a href="?featured=1&id=<?php echo base64_encode($row['id']); ?>"  title="NO">No</a>
                  <?php } ?>
                </td>                

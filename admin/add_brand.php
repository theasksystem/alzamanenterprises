...
$getResult = $conn->prepare("SELECT * FROM `products_brand` WHERE `id` = :id");
...
if(isset($_REQUEST['submit'])){
	...
	$sql = $conn->prepare("INSERT INTO `products_brand`(`brand`, `created_at`) VALUES (:brand, :created_at)");
...
if(isset($_REQUEST['update'])){
	...
	$sql = $conn->prepare("UPDATE `products_brand` SET `brand` = '$brand', `created_at` = now() WHERE `id` = '$id'");
	$sql->execute();
		
	if($sql == true){
		
			echo '<script>alert("Data has been Updated Successfully !!")</script>';
			echo '<script>window.location.href="view-brand.php"</script>';
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <?php if ($id == ''){ echo 'Add Brand'; } else{echo 'Update Brand';} ?>
      </h1>
      <ol class="breadcrumb">
            ...
            <div class="col-md-12">
              ...
              <div class="box box-primary">
                <form role="form" name="stform" id="stform" action="" method="post" enctype="multipart/form-data">
                  <div class="form-group">
                    <label>Brand</label>
                    <input class="form-control" placeholder="Brand"  name="brand" value="<?=$editval['brand']; ?>" required>
                  </div>

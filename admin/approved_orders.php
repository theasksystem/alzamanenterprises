...
<?php
$i=1;
$query = $conn->prepare("select c.id,c.status,c.created_at,c.user_id,c.address_id,c.total, p.user_id as vendorId from cart_orders c LEFT JOIN products p ON p.id = c.pid  WHERE c.status = 'approved' order by id desc");

...
else
{
  $slug = strtolower(trim(preg_replace("/[\s-]+/", "-", preg_replace( "/[^a-zA-Z0-9\-]/", '-', addslashes($_POST['product_name_en']."-".time()))),"-"));
		...
		$sql = $conn->prepare("INSERT INTO `products`(`user_id`, `tab_id`, `cat_id`, `subcat_id`, `product_name_en`, `product_name_ar`, `short_desc_en`, `short_desc_ar`, `long_desc_en`, `long_desc_ar`, `product_spec_en`, `product_spec_ar`, `image`, `discount_type`, `discount`, `price`, `old_price`, `slug`, `insert_ip`, `created_at`, `featured`,`p_size`, `ship_charge`, `free_shipping_amount`, `free_shipping_qty`) VALUES (:user_id, :tab_id, :cat_id, :subcat_id, :product_name_en, :product_name_ar, :short_desc_en, :short_desc_ar, :long_desc_en, :long_desc_ar, :product_spec_en, :product_spec_ar, :image, :discount_type, :discount, :price, :old_price, :slug, :insert_ip, :created_at, :featured, :size, :ship_charge, :free_shipping_amount, :free_shipping_qty)");
		...
		$sql->bindParam(':discount_type', $discount_type, PDO::PARAM_INT);
		...
		$sql->bindParam(':featured', $featured, PDO::PARAM_STR);
...
else
{
  $slug = strtolower(trim(preg_replace("/[\s-]+/", "-", preg_replace( "/[^a-zA-Z0-9\-]/", '-', addslashes($_POST['product_name_en']."-".time()))),"-"));
		...
		$sql = $conn->prepare("UPDATE `products` SET  `tab_id` = :tab_id, `cat_id` = :cat_id, `subcat_id` = :subcat_id, `product_name_en` = :product_name_en, `product_name_ar` = :product_name_ar, `short_desc_en` = :short_desc_en, `short_desc_ar` = :short_desc_ar, `long_desc_en` = :long_desc_en, `long_desc_ar` = :long_desc_ar, `product_spec_en` = :product_spec_en, `product_spec_en` = :product_spec_en, `image` = :image, `discount_type` = :discount_type, `discount` = :discount, `price` = :price, `old_price` = :old_price, `p_size` = :size, `featured` = :featured, `slug` = :slug, `update_ip` = :update_ip, `updated_at` = :updated_at, `ship_charge` = :ship_charge, `free_shipping_amount` = :free_shipping_amount, `free_shipping_qty` = :free_shipping_qty WHERE `id` = :id");

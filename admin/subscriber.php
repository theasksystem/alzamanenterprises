...
if (isset($_GET['visible'])) {
     $result = $conn->prepare("update usersubscribe set visible = :visible where id = :id");
...
if(isset($_REQUEST['delete']) && $_REQUEST['delete']=='y'){
	...
	$delUser = $conn->prepare("DELETE FROM `usersubscribe` WHERE `id` = :id");
            ...
            <tbody>
              <?php
				...
				$i=1;
					...
					$query = $conn->prepare("select * from usersubscribe order by id desc");
					$query->execute();

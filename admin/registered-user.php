...
if (isset($_GET['visible'])) {
     $result = $conn->prepare("update registration set visible = :visible where id = :id");
	 $result->bindParam(':visible', $_GET['visible'], PDO::PARAM_INT);
	 $result->bindParam(':id', $id, PDO::PARAM_INT);
	 $result->execute();

    echo "<script>window.location='registered-user.php';</script>";
}


if(isset($_REQUEST['delete']) && $_REQUEST['delete']=='y'){

	$delUser = $conn->prepare("DELETE FROM `registration` WHERE `id` = :id");
...
<title>All | Registered Users list</title>
            ...
            <tbody>
              <?php
				...
				$i=1;
					...
					$query = $conn->prepare("select * from registration order by id desc");
					$query->execute();

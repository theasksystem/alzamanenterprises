...
$query = $conn->prepare("select user_id,id,created_at,status from cart_orders WHERE status = 'completed' and id IN(select order_id from cart_order_item where pid IN(select id from products where user_id='".$_SESSION['VENDOR_ID']."')) order by id desc");

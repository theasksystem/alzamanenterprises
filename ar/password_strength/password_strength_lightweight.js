...
;(function ( $, window, document, undefined ) {
    ...
    Plugin.prototype._create = function () {
        ...
        this.element.append('<input type="password" id="password" placeholder="كلمة المرور" name="pass" class="form-control ' + options.inputClass + '" "/>');
    ...
    Plugin.prototype._update_indicator = function(total) {
        ...
        if (total === 0) {
            ...
            meter.addClass('veryweak').html('very weak');
        } else if (total === 2) {
            meter.addClass('weak').html('weak');
        } else if (total === 3) {
            meter.addClass('medium').html('medium');
        } else {
            meter.addClass('strong').html('strong');
        }
    },
